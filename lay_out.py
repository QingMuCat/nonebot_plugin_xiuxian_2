...
from nonebot.adapters.onebot.v11 import Bot, MessageSegment
from .xiuxian_config import XiuConfig, JsonConfig
from .utils import get_msg_pic,get_msg_pic2
    ...
    async def dependency(bot: Bot, matcher: Matcher, event: MessageEvent):
        ...
        if running[key] <= 0:
            if cd_time >= 1.5:
                ...
                if time <= 1:
                    time = 1
                msg2 = await get_msg_pic2(f"@{event.sender.nickname}\n" + _chat_flmt_notice.format(time))
                if global_send_cd_limit:
                    bot = await assign_bot_group(group_id=group_id)
                    await bot.send_group_msg(group_id=int(group_id), message=msg2)
                    await matcher.finish()
                else:
                    await matcher.finish(message=msg2, at_sender=at_sender)
            else:
                await matcher.finish()
        else:
            time_sy[key] = int(loop.time())
